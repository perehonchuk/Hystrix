...
public class RequestCollapser<BatchReturnType, ResponseType, RequestArgumentType> {
    ...
    RequestCollapser(HystrixCollapserBridge<BatchReturnType, ResponseType, RequestArgumentType> commandCollapser, HystrixCollapserProperties properties, CollapserTimer timer, HystrixConcurrencyStrategy concurrencyStrategy) {
        ...
        this.timer = timer;
        batch.set(createNewBatch());
    }

    private RequestBatch<BatchReturnType, ResponseType, RequestArgumentType> createNewBatch() {
        rx.functions.Action0 immediateExecutionCallback = new rx.functions.Action0() {
            @Override
            public void call() {
                // Trigger immediate execution by creating a new batch
        ...
        return new RequestBatch<BatchReturnType, ResponseType, RequestArgumentType>(properties, commandCollapser, properties.maxRequestsInBatch().get(), immediateExecutionCallback);
    }
    ...
    private void createNewBatchAndExecutePreviousIfNeeded(RequestBatch<BatchReturnType, ResponseType, RequestArgumentType> previousBatch) {
        ...
        if (batch.compareAndSet(previousBatch, createNewBatch())) {
