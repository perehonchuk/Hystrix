...
public class RequestBatch<BatchReturnType, ResponseType, RequestArgumentType> {
    ...
    public void executeBatchIfNotAlreadyStarted() {
        ...
        if (batchStarted.compareAndSet(false, true)) {
            ...
            try {
                // sort batch requests first
                Collection<CollapsedRequest<ResponseType, RequestArgumentType>> sortedRequests = commandCollapser.sortBatchRequests(argumentMap.values());
                // then shard batches
