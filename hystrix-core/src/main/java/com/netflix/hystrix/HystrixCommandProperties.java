...
public abstract class HystrixCommandProperties {
    ...
    private static final Integer default_requestCacheTTLInMilliseconds = -1; // default to -1 (disabled, request-scoped only)
    ...
    private final HystrixProperty<Integer> requestCacheTTLInMilliseconds; // Time-to-live in milliseconds for cache entries (-1 means request-scoped only)
    ...
    /**
     * Time-to-live in milliseconds for cached entries in {@link HystrixRequestCache}.
     * A value of -1 means cache entries are request-scoped only (default behavior).
     * A value >= 0 means cache entries will expire after the specified time, even within the same request context.
     *
     * @return {@code HystrixProperty<Integer>}
     */
