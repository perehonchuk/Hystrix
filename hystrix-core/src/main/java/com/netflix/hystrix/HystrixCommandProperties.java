...
public abstract class HystrixCommandProperties {
    ...
    private static final Integer default_fallbackMaxRetryAttempts = 3; // default => retry fallback up to 3 times on failure
    ...
    private final HystrixProperty<Integer> fallbackMaxRetryAttempts; // Maximum number of retry attempts for fallback execution on failure
     ...
     * When a fallback throws an exception, Hystrix will retry the fallback up to this many times
     * before ultimately failing. A value of 0 disables retry (single attempt only).
