...
public abstract class HystrixCommandProperties {
    ...
    private static final Boolean default_retryEnabled = true; // default => retry is enabled
    private static final Integer default_retryMaxAttempts = 2; // default => retry up to 2 times (3 total attempts including the first)
    private static final Integer default_retryDelayInMilliseconds = 100; // default => wait 100ms between retry attempts
    ...
    private final HystrixProperty<Boolean> retryEnabled; // Whether automatic retry is enabled on execution failures.
    private final HystrixProperty<Integer> retryMaxAttempts; // Maximum number of retry attempts (not counting initial attempt).
    ...
    /**
     * Whether automatic retry is enabled for failed executions.
     * <p>
     * When enabled, if a command execution fails (not including fallback or circuit breaker scenarios),
     * Hystrix will automatically retry the execution up to {@link #retryMaxAttempts()} times with a
     * delay of {@link #retryDelayInMilliseconds()} between attempts.
     *
     * @return {@code HystrixProperty<Boolean>}
     */
    ...
    /**
     * Maximum number of retry attempts for failed executions (not counting the initial attempt).
     * <p>
     * For example, if set to 2, Hystrix will attempt execution up to 3 times total (1 initial + 2 retries).
     * This only applies when {@link #retryEnabled()} is true.
     *
     * @return {@code HystrixProperty<Integer>}
     */
     ...
     * <p>
     * When a command execution fails and retry is enabled, Hystrix will wait this amount of time
     * before attempting the next retry.
     *
     * @return {@code HystrixProperty<Integer>}
     */
