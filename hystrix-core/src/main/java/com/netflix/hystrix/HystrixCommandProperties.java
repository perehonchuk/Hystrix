...
public abstract class HystrixCommandProperties {
    ...
    private static final Integer default_circuitBreakerHalfOpenMaxConcurrentRequests = 1;// default => halfOpenMaxConcurrentRequests: 1 = only one request allowed during half-open recovery phase
    private static final Integer default_circuitBreakerHalfOpenSuccessThreshold = 1;// default => halfOpenSuccessThreshold: 1 = number of successful requests needed to close circuit from half-open state
    ...
    private final HystrixProperty<Integer> circuitBreakerHalfOpenMaxConcurrentRequests; // maximum number of concurrent requests allowed during half-open recovery phase
    private final HystrixProperty<Integer> circuitBreakerHalfOpenSuccessThreshold; // number of consecutive successful requests needed to transition from half-open to closed
    ...
    /**
     * Maximum number of concurrent requests permitted during the half-open recovery phase.
     * When set to 1 (default), only a single request is allowed to test service recovery.
     * Higher values allow progressive recovery with multiple concurrent test requests.
     *
     * @return {@code HystrixProperty<Integer>}
     */
    ...
    /**
     * Number of consecutive successful requests needed to transition from half-open to closed state.
     * When set to 1 (default), a single successful request closes the circuit.
     * Higher values require multiple consecutive successes before fully recovering.
     *
     * @return {@code HystrixProperty<Integer>}
     */
